...
- Market sell and the control-room market panel now keep zero-demand rows visible when the station still exposes a sell price, and they order those rows after the normal demand list to match the in-game SELL tab so cursor indexing stays aligned for cargo like `Food Cartridges`.
