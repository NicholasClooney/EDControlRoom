...
Last updated: 2026-06-06 (session 6)
...
| Community haul loop | Done (not live-validated) | `edap/routines.py` — `haul_loop` chains existing pieces: sell all cargo from `Cargo.json` (skips stolen/mission), undock, wait SCX + dock at buy station, buy MAX commodity, undock, wait SCX + dock at sell station with auto-refuel, repeat; wired to `run_routine.py` as `--routine haul_loop --target <commodity> [--buy-station NAME] [--sell-station NAME] [--iterations N]` (0 = infinite) |
| Control room TUI | Done (not live-validated) | `control_room.py` — Textual TUI with ship status, activity log, and market panels; dispatches `dock`/`undock`/`jump`/`buy`/`sell`/`haul` routines from the input bar; `sell` with no args iterates full cargo manifest; `haul [commodity]` prompts step-by-step for commodity, buy station, sell station (stations optional); enforces start-at-sell-station precondition (blocks if not docked, warns if station mismatch); `verbose on/off` toggles key press logging to the activity log; key dispatch shared via `edap/progress_controls.py` (`ProgressShipControls`, `verbose=False` default) |
