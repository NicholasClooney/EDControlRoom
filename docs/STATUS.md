...
| Galaxy map destination | **Needs live validation** | `edap/routines.py` — `set_gal_map_destination` opens map, types system name, picks search result, plots with `CamZoomIn + UI_Select(held)`, verifies `NavRoute.json`, retries on mismatch; wired to `run_routine.py` as `--routine set_gal_map_destination --destination <SYSTEM>`; `type_text` added to `MacOSInputController` via `CGEventKeyboardSetUnicodeString`. Unit tests pass. First live test: `python run_routine.py --routine set_gal_map_destination --destination "Colonia" --delay-seconds 5` |
| Control room TUI | **Needs live validation** | `control_room.py` — Textual TUI with ship status, activity log, and market panels; dispatches `dock`/`undock`/`jump`/`buy`/`sell`/`haul` routines from the input bar; `sell` with no args iterates full cargo manifest; `haul [commodity]` prompts step-by-step for commodity, buy station, sell station (stations optional); enforces start-at-sell-station precondition (blocks if not docked, warns if station mismatch); `verbose on/off` toggles key press logging to the activity log; key dispatch shared via `edap/progress_controls.py` (`ProgressShipControls`, `verbose=False` default) |
...
- **Galaxy map input.** Implemented as `set_gal_map_destination` in `edap/routines.py`. Live validation pending: OCR-based open detection (`open_check_fn`) is optional; defaults to a settle delay. The `control_room.py` `go <system>` command is the remaining gap.
