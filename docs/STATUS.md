...
- `control_room.py` now persists operator state to a local JSON file (`control_room.state_file`, default `.control_room_state.json`): saved haul defaults survive restart, destination settle overrides persist, and recent command history is retained across sessions (`control_room.history_limit`, default 20)
- `resume` is now implemented in `control_room.py` as a scrollable picker backed by structured saved history: Enter re-executes the selected command immediately, `e` reopens it for editing, and haul replay/edit restores the saved multi-step prompt defaults instead of dropping back to a raw command line
...
| Control room TUI | Done | `control_room.py` — Textual TUI with ship status, activity log, and market panels; the left pane is wider, and the activity log is back on `RichLog` so the original rich colors/highlighting are preserved. `dest <system>` / `set_dest <system>` prompts for galaxy-map settle seconds (Enter = saved override or config default). `haul [commodity]` prompts step-by-step for commodity, buy station, sell station, systems, galaxy-map settle seconds, and haul docking timeout seconds, now reusing saved defaults from the local control-room state file across restarts. `resume` opens a scrollable recent-command picker (cross-session history, default retention 20) where Enter re-executes and `e` reopens for edit; haul edit restores the structured prompt flow. Sell station defaults to current docked station; start-at-sell-station precondition still enforced. `verbose on/off` toggles key press logging; command history (up/down arrows); `commands` lists supported commands; `help [command]` explains them in plain English; Ctrl-C/Ctrl-D now cancel the active routine worker and keep control room open when a routine is running, and only trigger app shutdown when idle; input bar auto-focused on launch; `market filter <name>` sets filter (title-cased); `market` / `market clear` clears; `market lock/unlock`. Sell-all now falls back to `Cargo.json` if no fresh `Cargo` journal event has populated the live manifest yet. Local tests cover command/help dispatch, interrupt/shutdown ordering, config defaults, history trimming, and control-room state round-tripping. Interrupt handling is live-validated in the real `uv run python3 control_room.py` flow; persisted-default and `resume` UX still need live validation. |
...
- **Control room persistence follow-up.** Local JSON-backed operator state and the `resume` picker are now implemented in `control_room.py`. Next live check: verify the modal UX, Enter-to-execute path, `e` edit path, and persisted haul defaults in the real `uv run python3 control_room.py` loop against Elite + CrossOver.
